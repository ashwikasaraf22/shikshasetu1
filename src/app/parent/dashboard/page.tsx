...
/* -----------------------------
   ...
   (FUNCTIONS UNCHANGED in behavior)
----------------------------- */

// ✅ NOTE: I only add "studentID" to the list so it matches your data.
// This does not change function names/signatures or how you call anything.
async function getDocsByAnyStudentField(
  coll: string,
  childUid: string,
  fieldCandidates: string[] = ["studentId", "studentID", "userId", "uid"] // includes studentID (capital D)
) {
...
// Normalize timestamps from various shapes (UNCHANGED)
...
async function fetchTeachersTests(childUid: string, since: Date): Promise<Row[]> {
  const docs = await getDocsByAnyStudentField("community_test_answers", childUid, [
    "studentId", // your screenshots show lowercase d here
    "studentID",
    "userId",
    "uid",
  ]);
...
export default function ParentDashboard() {
  ...
  // Keep workshops/videos present (disabled) per your request
  const kindOptions: { value: ActivityKind | "workshops" | "videos"; label: string; disabled?: boolean }[] = [
    { value: "chapters", label: "📘 Chapters Read" },
    { value: "quizzes", label: "🧠 Quiz Taken" },
    { value: "teachers", label: "🏫 Teacher’s Test" },
    { value: "workshops", label: "🎉 Workshops Attended", disabled: true },
    { value: "videos", label: "🎬 Videos Watched", disabled: true },
  ];
        ...
        <Card>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="flex flex-col">
              ...
              {(kind === "chapters" || kind === "quizzes" || kind === "teachers") ? null : (
