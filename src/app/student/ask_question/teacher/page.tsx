...
import { collection, query as fq, where, orderBy, onSnapshot, serverTimestamp, addDoc, getDocs, doc } from 'firebase/firestore';
...
export default function AskTeacherPage() {
  ...
  const chapters: Record<string, string[]> = chaptersByClass[resolvedClass as '7' | '8' | '9'] || chaptersByClass['9'];
  ...
  useEffect(() => {
    if (!teacherSubject) { setTeachers([]); setSelectedTeacher(''); return; }

    const fetchTeachers = async () => {
      try {
        const q = fq(collection(db, 'users'), where('role', '==', 'teacher'), where('subject', '==', teacherSubject));
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(qWithSort, async (snap) => {
  ...
  // Call active listener
  useEffect(() => {
    if (!selectedDoubt?.id) { setCallActive(false); return; }
  ...
  const handleSubmitToTeacher = async () => {
    if (!teacherSubject || !teacherChapter || !teacherQuestion.trim() || !selectedTeacher) return alert('Please fill all fields and select a teacher.');
    try {
      setSaving(true);
      ...
      setTeacherQuestion(''); setSelectedTeacher('');
  ...
  const openDoubtDetails = (d: any) => { setSelectedDoubt(d); setNewFollowup(''); };
  const handleSendFollowup = async () => {
    if (!selectedDoubt || !newFollowup.trim()) return;
    try {
      await addStudentMessage({ doubtId: selectedDoubt.id, studentId: user!.uid, text: newFollowup.trim(), language: selectedDoubt.language || selectedLanguage });
      setNewFollowup('');
    } catch (e: any) { alert(e?.message || 'Failed to send your message.'); }
  };
          ...
          <>
            ...
            <Card className="bg-white/90 backdrop-blur shadow-2xl border-white/70">
              <CardHeader>
                ...
                <CardDescription>Select your subject, chapter, teacher, and describe your doubt.</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <Select value={teacherSubject} onValueChange={setTeacherSubject}>
                    <SelectTrigger><SelectValue placeholder="Select Subject" /></SelectTrigger>
                    <SelectContent>{Object.keys(chapters).map(s => <SelectItem key={s} value={s}>{s}</SelectItem>)}</SelectContent>
                  </Select>

                  <Select value={teacherChapter} onValueChange={setTeacherChapter} disabled={!teacherSubject}>
                    <SelectTrigger><SelectValue placeholder="Select Chapter" /></SelectTrigger>
                    <SelectContent>{teacherSubject && chapters[teacherSubject]?.map(c => <SelectItem key={c} value={c}>{c}</SelectItem>)}</SelectContent>
                  </Select>

                  <Select value={selectedTeacher} onValueChange={setSelectedTeacher} disabled={teachers.length === 0}>
                ...
                <Textarea rows={4} placeholder="Type your question…" value={teacherQuestion} onChange={(e) => setTeacherQuestion(e.target.value)} />

                <Button onClick={handleSubmitToTeacher} disabled={saving} className="bg-gradient-to-r from-[#6F5AE8] via-[#7EC8E3] to-[#F7AFC1] text-white hover:brightness-110">
            ...
            <section className="mt-8">
              ...
              <div className="grid gap-4 md:grid-cols-2">
                {submittedQuestions.map(q => (
                  <Card key={q.id} className="bg-white/90 backdrop-blur shadow-lg border-white/60">
                    <CardContent>
                      <p className="font-semibold text-[#3F338C]">{q.question}</p>
                      <p className="mt-1 text-sm text-[#5A4DA8]/80">
                        Subject: {q.subject} • Chapter: {q.chapter} • Teacher: {q.teacherName || 'Unknown'} • Status: {q.status}
                      </p>
                    </CardContent>
                  </Card>
                ))}
                {submittedQuestions.length === 0 && <Card className="bg-white/80 border-dashed border-[#D5CEFF]"><CardContent>No questions submitted yet.</CardContent></Card>}
              </div>
            </section>
          </>
        )}
