...
import { CalendarClock, FileUp, Video, Users, X } from 'lucide-react'; // +X to close modal
import { useMemo, useState } from 'react';

/** Firebase (uses your existing /lib/firebase.ts) */
...
function TeacherCommunity() {
  ...
  const tabs = [
    ...
    },
    {
      ...
      link: '#',
      onClick: 'open-upload' as const, // intercept click to open modal here
    },
    {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // Storage path: videos/<teacherUid>/<timestamp>_<sanitized-title>.<ext>
      ...
      await addDoc(collection(db, 'videos'), {
        ...
        languageCode: language,           // filter key for students
        languageLabel: selectedLangLabel, // for display
        storagePath: path,
        downloadURL,
        size: file.size,
      ...
      {showUpload && (
        <div className="fixed inset-0 z-30 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-2xl rounded-3xl bg-white/90 backdrop-blur-lg border border-white/60 shadow-2xl p-6 relative">
            ...
            <h3 className="text-2xl font-extrabold text-[#6B5BBE] mb-1">Upload a Video</h3>
            ...
            <form onSubmit={onSubmit} className="space-y-4">
              {/* Language */}
              <div>
                <label className="block text-sm font-medium text-[#4E3FA3] mb-1">Language</label>
                <select
                  value={language}
              ...
              {/* Title */}
              <div>
                <label className="block text-sm font-medium text-[#4E3FA3] mb-1">Title</label>
                <input
                  type="text"
                  value={title}
              ...
              {/* Description */}
              <div>
                <label className="block text-sm font-medium text-[#4E3FA3] mb-1">Description (optional)</label>
                <textarea
                  value={desc}
              ...
              {/* File */}
              <div>
                <label className="block text-sm font-medium text-[#4E3FA3] mb-1">Video file</label>
                <input
                  type="file"
                  accept="video/*"
                  onChange={onPickFile}
                  ...
                  disabled={uploading}
                />
                <p className="text-xs text-[#5A4DA8]/70 mt-1">Accepted: MP4, WebM, MOV, etc.</p>
              </div>
